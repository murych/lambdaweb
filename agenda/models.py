...
class Event(Displayable, Ownable, RichText, AdminThumbMixin):
    ...
    facebook_event = models.BigIntegerField(_('Facebook'), blank=True,
                                            null=True)  #
    ...
    featured_image = FileField(verbose_name=_("Featured Image"),
                               upload_to=upload_to(
                                   "agenda.Event.featured_image", "event"),
                               format="Image", max_length=255, null=True,
                               blank=True)
...
class EventLocation(Slugged):
    ...
    mappable_location = models.CharField(max_length=128, blank=True,
    ...
    def clean(self):
        ...
        if self.mappable_location and not (
            self.lat and self.lon):  # location should always override lat/long if set
            g = GoogleMaps(domain=settings.EVENT_GOOGLE_MAPS_DOMAIN)
            try:
                mappable_location, (lat, lon) = g.geocode(
