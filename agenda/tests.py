...
from agenda.models import Event, EventLocation
...
class EventTests(TestCase):
    def setUp(self):
        ...
        self.event_page = RichTextPage.objects.create(title="events",
    ...
    def test_event_views(self):
        ...
        self.assertEqual(response.status_code, 200)
        event = Event.objects.create(title="Event", start=datetime.now(),
                                     user=self._user,
                                     status=CONTENT_STATUS_PUBLISHED)
    ...
    def test_clean(self):
        ...
        self.assertEqual(self.eventlocation.mappable_location,
                         '1 Susan Street, Hindmarsh SA 5007, Australia')
        self.unicode_eventlocation.clean()
        self.assertAlmostEqual(self.unicode_eventlocation.lat, 35.729534,
                               places=5)
        self.assertAlmostEqual(self.unicode_eventlocation.lon, 139.718055,
                               places=5)
    ...
    def test_icalendars(self):
        ...
        for event in self.events:
            response = self.client.get(
                reverse("icalendar_event", args=(event.slug,)))
            self.assertEqual(response.status_code, 200)
